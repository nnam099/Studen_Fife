...
public interface TimeDao {
    ...
    @Query("SELECT * FROM tasks WHERE user_id = :userId AND deleted = 0 ORDER BY deadline ASC")
    List<TaskEntity> getActiveTasks(String userId);

    @Query("SELECT * FROM tasks WHERE user_id = :userId AND status = :status AND deleted = 0 ORDER BY deadline ASC")
    List<TaskEntity> getActiveTasksByStatus(String userId, String status);
    ...
    @Query("SELECT * FROM calendar_events WHERE user_id = :userId AND deleted = 0 AND start_time >= :startTime AND end_time <= :endTime ORDER BY start_time ASC")
    List<CalendarEventEntity> getEventsInRange(String userId, long startTime, long endTime);
